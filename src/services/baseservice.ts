...
export class BaseService {
    ...
    protected generatePrettyName(entity) {
        let entityName = "";
        entityName = entity?.secret?.name || entity?.name;
        entity?.office ? entityName = entity?.office + "-" + entityName : "";
        entityName = entityName.toLowerCase();
        if (entityName) {
            entityName = entityName.replace(/,/g, '');
            entityName = entityName.replace(/ /g, ',');
            entityName = entityName.replace(/[^0-9A-Za-z,-]/g, '');
            entityName = entityName.replace(/,/g, '-');
            entityName = entityName.replace(/-{2,}/g, '-');
        }
        return entityName;
    }
    ...
    protected async updateEntitiesWithItsPrettyName(req) {
        ...
        ["media", "minister", "mediatag"].forEach(async section => {
            ...
            await req.app.locals[section]
                ...
                .then(result => {
                    if (result) {
                        existingEntityWithEmptyPrettyName = result;
                    }
                });

            let that = this;
            if (existingEntityWithEmptyPrettyName && existingEntityWithEmptyPrettyName.length) {
                existingEntityWithEmptyPrettyName.forEach(async entity => {
                    let prettyName = await that.generatePrettyName(entity);
